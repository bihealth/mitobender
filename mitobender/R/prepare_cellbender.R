...
option_list <- list(
  ...
  make_option("--strand_correlation",type="double",default=.5,help="strand_correlation to select variable positions [%default]",metavar="numeric"),
  make_option("--vmr",type="double",default=.001,help="vmr to select variable positions [%default]",metavar="numeric"),
  make_option("--variable_sites",type="character",help="file with list of variable positions to use (one per line)",metavar="file")
)
...
if (is.null(options$variable_sites)) {
  message("determining variable sites by cutoffs")
  var.pos <- subset(variable.sites, subset = (n_cells_conf_detected >= options$n_cells_conf_detected & strand_correlation >= options$strand_correlation & vmr > options$vmr))
} else {
  message("using variable sites from file")
  variants <- readLines(options$variable_sites)
  var.pos <- subset(variable.sites, subset = (variant %in% variants))
} 
